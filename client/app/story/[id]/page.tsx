...
import type { StoryPage, StoryContext } from '@/lib/types';
...
export default function StoryPage({ params }: PageProps) {
  const [story, setStory] = useState(fakeStory);
  ...
  useEffect(() => {
    async function fetchStory() {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001'}/api/story/${params.id}`);
        const fetchedStory = await response.json();

        if (response.ok) {
          setStory(fetchedStory);
        } else {
          console.warn('Failed to fetch story, using fake story:', fetchedStory.error);
          setStory(fakeStory);
        }
      } catch (error) {
  ...
  // Story context for voice conversation
  const storyContext: StoryContext = {
    topic: story.topic,
    current_page: currentPage.page_id,
    branch_taken: null, // TODO: Track branch choices
    story_so_far: pages.slice(0, currentIndex + 1).map(p => p.narration).join(' '),
    narrator_character: story.narrator.character,
    educational_facts_covered: [], // TODO: Track facts mentioned
    adhd_mode: true
  };
  ...
  const voiceConversation = useVoiceConversation({
    storyContext,
    autoConnect: false // Only connect when user wants to talk
  });

  // ── Audio playback with fallback to speech synthesis ─────────────────────
  ...
  useEffect(() => {
    ...
    if (currentPage.audio_url) {
      playAudio(currentPage.audio_url);
    } else {
  ...
  function handleTalk() {
    if (!voiceConversation.isConnected) {
      // Connect to voice service first
      voiceConversation.connect()
        ...
        .catch((error) => {
          ...
          speak("I'm having trouble connecting to the voice service. Let me try again in a moment!");
        });
    } else {
      setShowVoiceConversation(true);
    }
  ...
  if (storyLoading) {
    return (
      <StoryViewer title="Loading...">
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          <div className="text-6xl mb-4 animate-spin">⚙️</div>
          <h2 className="text-2xl font-bold text-white mb-2">Creating Your Story...</h2>
          <p className="text-gray-400">This might take a moment while we generate your personalized adventure!</p>
        </div>
      </StoryViewer>
    );
  }
  ...
  return (
    <StoryViewer title={story.title}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <NarratorPanel
              ...
              isSpeaking={isSpeaking || voiceConversation.isSpeaking}
              audioLoading={audioLoading}
              audioError={audioError}
              conversationState={{
                isConnected: voiceConversation.isConnected,
      ...
      {/* Hidden audio element for ElevenLabs audio playback */}
      <audio
        ref={audioRef}
